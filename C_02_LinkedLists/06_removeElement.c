/*
Remove last element froma linked list
*/
...
struct node *remove_at_end(struct node *head);
struct node *remove_at_end2(struct node *head);
...
int count_nodes (struct node *head) //we pass to the function head (a pointer)
{
    int count = 0;
    if (head == NULL){
        printf("List empty\n");
...
struct node *remove_at_end(struct node *head){//using two pointers (ptr and temp). Type struct in order to return head in case of only 1 element list
    if (head == NULL){
        printf("Empty list!\n");
        ...
        temp->link = NULL;
        free(ptr);//this only free the memory
        ptr = NULL;//this "destroy" the ptr           
    }
}

//alternative using single pointer instead of two
struct node *remove_at_end2(struct node *head){//using two pointers (ptr and temp)
    if (head == NULL){
        printf("Empty list!\n");
    }else if (head->link == NULL){
        free(head);
        ...
        free(ptr->link);//this only free the memory
        ptr->link = NULL;
    }   
}//CONTROLLARE!!!